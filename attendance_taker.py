...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "Face Recognisation ", (20, 40), self.font, 1, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def attendance(self, name):
        ...
        #existing_entry = cursor.execute("SELECT * FROM attendance WHERE name = ? AND date = ?", (name, current_date)).fetchone()

        #if existing_entry:
            # Update the existing entry if needed
            #cursor.execute("UPDATE attendance SET time = ? WHERE name = ? AND date = ?", (current_time, name, current_date))
            #cursor.execute("INSERT INTO attendance (name, time, date) VALUES (?, ?, ?)", (name, current_time, current_date))
            #
         #   print(f"{name} already present for {current_date} at {current_time}")
        #else:
            # Insert a new entry
        cursor.execute("INSERT INTO attendance (name, time, date) VALUES (?, ?, ?)", (name, current_time, current_date))
